...
const App = () => {
  ...
  const handleClick = (e) => {
    setResult(reuslt.concat(e.target.name))
  }
  const clear = () => {
    setResult("");
  }
  const backspace = () => {
    setResult(reuslt.slice(0, -1))
  }
  const calculate = () => {

    try {
      setResult(eval(reuslt).toString());
    }
    catch(err){
      setResult("Error")
    }
  ...
  return(
    <>
     <div className="container">
      <form>
        <input type="text" value={reuslt} />
      </form>
      <div className="keypad">
        <button className="operator" onClick={clear} id="clear">AC</button>
        <button className="operator" onClick={backspace} id='backspace'>DE</button>
